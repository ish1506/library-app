...
- Store positive integer reservation timing in `backend/config/library.yaml` under `reservations.hold_seconds` and `reservations.worker_interval_seconds`, defaulting to `86_400` and `60`. Existing reservations retain their persisted expiry timestamp if this setting later changes.
