...
describe("App", () => {
  ...
  it("loads a user catalogue without mutation controls", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Add book" }),
  ...
  it("navigates from the user loans page back to the catalogue", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "My loans" }),
    ).toBeInTheDocument();
  ...
  it("applies combined text and inclusive date filters", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "No matching books" }),
    ).toBeInTheDocument();
  });
  ...
  it('lets a user reserve an unavailable title', async () => {
    const book = { id: 1, title: 'Dune', author: 'Frank Herbert', date: 0, isbn: '9780441013593', loan_duration_days: 14, total_copies: 1, available_copies: 0, late_fee_cents_per_day: 50 }
  ...
  it("lets an admin view loans for a book from the detail view", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "View loan history" }),
    );

    expect(
      await screen.findByRole("heading", { name: "Loan history" }),
    ).toBeInTheDocument();
    expect(screen.getByText("Loan #4")).toBeInTheDocument();
    expect(listBookLoansMock).toHaveBeenCalledWith(
      "header.eyJyb2xlIjoiQURNSU4ifQ==.signature",
      1,
    );
  });
});
