...
function errorMessage(status: number, body: unknown): string {
  ...
  if (status === 422) return detail || 'The book details were not accepted. Check the form and try again.'
  return detail || 'The library service could not complete that request. Please try again.'
}
...
async function request<T>(path: string, token: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new BooksApiError(0, 'Unable to reach the library service. Check that it is running and try again.')
  }

  const body = response.status === 204 ? undefined : await readJson(response)
  if (!response.ok) throw new BooksApiError(response.status, errorMessage(response.status, body))
  return body as T
}
...
export async function getBook(token: string, id: number): Promise<Book> {
  ...
  if (!isBook(body)) throw new BooksApiError(0, 'The library service returned an unexpected book response.')
  return body
}

export async function createBook(token: string, book: BookCreate): Promise<Book> {
  const body = await request<unknown>('/books', token, {
    method: 'POST',
...
export async function updateBook(token: string, id: number, changes: BookUpdate): Promise<Book> {
  const body = await request<unknown>(`/books/${id}`, token, {
    method: 'PATCH',
