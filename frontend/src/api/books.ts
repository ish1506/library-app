...
function errorMessage(status: number, body: unknown): string {
  ...
  if (status === 422)
    return detail || 'The book details were not accepted. Check the form and try again.'
...
export async function getBook(token: string, id: number): Promise<Book> {
  ...
  if (!isBook(body))
    throw new BooksApiError(0, 'The library service returned an unexpected book response.')
  return body
}
