...
function errorMessage(status: number, body: unknown): string {
  ...
  return detail || 'The library service could not complete that reservation request. Please try again.'
}

async function request<T>(path: string, token: string, method: 'GET' | 'POST' | 'DELETE'): Promise<T> {
  ...
  try {
    ...
    throw new ReservationsApiError(0, 'Unable to reach the library service. Check that it is running and try again.')
  }

  const body = response.status === 204 ? undefined : await readJson(response)
  if (!response.ok) throw new ReservationsApiError(response.status, errorMessage(response.status, body))
  return body as T
}
...
export async function createReservation(token: string, bookId: number): Promise<BookReservation> {
  ...
  if (!isBookReservation(body)) throw new ReservationsApiError(0, 'The library service returned an unexpected reservation response.')
  return body
}
...
export async function listMyReservations(token: string): Promise<BookReservation[]> {
  ...
  if (!Array.isArray(body) || !body.every(isBookReservation)) {
    throw new ReservationsApiError(0, 'The library service returned an unexpected reservations response.')
  }
  return body
}

export async function confirmReservation(token: string, reservationId: number): Promise<ReservationConfirmation> {
  ...
  if (!isRecord(body) || !isBookReservation(body.reservation) || !isBookLoan(body.loan)) {
    throw new ReservationsApiError(0, 'The library service returned an unexpected confirmation response.')
  }
  return { reservation: body.reservation, loan: body.loan }
}

export async function cancelReservation(token: string, reservationId: number): Promise<BookReservation> {
