...
describe('loans API', () => {
  ...
  it('borrows without a request body and sends the bearer token', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(new Response(JSON.stringify(loan), { status: 201 }))
  ...
  it('validates the active loan list response', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify([loan]), { status: 200 }),
    )
  ...
  it('lists loans for a book with the bearer token', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response(JSON.stringify([loan]), { status: 200 }))
  ...
  it('normalizes conflicts and validates return responses', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ detail: 'Book is unavailable' }), {
        status: 409,
      }),
    )
