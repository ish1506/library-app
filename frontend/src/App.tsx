...
function App() {
  ...
  return <main className="app-shell">
    <header className="app-header"><div><p className="eyebrow">Library catalogue</p><h1>Books</h1></div><div className="header-actions"><span className="role-label">{currentRole === 'ADMIN' ? 'Administrator' : 'Member'}</span><button className="button-secondary" type="button" onClick={() => signOut()}>Sign out</button></div></header>
    ...
    {view === 'detail' && <section aria-labelledby="detail-heading"><button className="back-button" type="button" onClick={() => setView('list')}>Back to catalogue</button>{detailLoading && <p className="status" role="status">Loading book...</p>}{detailError && <div className="error-summary" role="alert">{detailError}</div>}{selectedBook && <><div className="detail-heading"><div><p className="eyebrow">Book details</p><h2 id="detail-heading">{selectedBook.title}</h2><p className="muted">{selectedBook.author}</p></div>{currentRole === 'ADMIN' && <div className="inline-actions"><button type="button" onClick={openEdit}>Edit</button><button className="button-danger" type="button" onClick={() => setDeleteConfirm(true)}>Delete</button></div>}</div><dl className="metadata"><div><dt>ISBN</dt><dd>{selectedBook.isbn}</dd></div><div><dt>Publication date</dt><dd>{formatPublicationDate(selectedBook.date)}</dd></div><div><dt>Loan duration</dt><dd>{selectedBook.loan_duration_days} days</dd></div><div><dt>Total copies</dt><dd>{selectedBook.total_copies}</dd></div><div><dt>Available copies</dt><dd>{selectedBook.available_copies}</dd></div><div><dt>Availability</dt><dd>{availability(selectedBook).label}</dd></div></dl></>}
    </section>}
