...
import { createReservation, listMyReservations, confirmReservation, cancelReservation, ReservationStatus, ReservationsApiError, type BookReservation } from './api/reservations'
import { listUnreadNotifications, markNotificationRead, NotificationsApiError, type Notification } from './api/notifications'
import './App.css'
...
function App() {
  ...
  async function refreshReservations(currentSession = session) {
    ...
    try {
      ...
      if (requestId === reservationRequestId.current) {
        ...
        const missingBookIds = [...new Set(nextReservations.map((reservation) => reservation.book_id))]
          .filter((bookId) => !books.some((book) => book.id === bookId) && !reservationBookTitles[bookId])
        const bookResults = await Promise.all(missingBookIds.map(async (bookId) => {
          try {
        ...
        if (requestId === reservationRequestId.current) {
          setReservationBookTitles((current) => Object.fromEntries([
            ...Object.entries(current),
            ...bookResults.filter((book): book is Book => book !== null).map((book) => [book.id, book.title]),
          ]))
        }
      }
    } catch (error) {
      ...
      else setReservationError(error instanceof Error ? error.message : 'Unable to load your reservations.')
    } finally {
  ...
  async function refreshNotifications(currentSession = session) {
    ...
    try {
      ...
      else setNotificationError(error instanceof Error ? error.message : 'Unable to load notifications.')
    }
  }

  useEffect(() => {
    if (!session || session.role !== 'USER') return
    const intervalId = window.setInterval(() => { void refreshNotifications(session) }, 10_000)
  ...
  async function refreshBookLoans(currentSession = session, book = selectedBook) {
    ...
    try {
      ...
      if (error instanceof LoansApiError && error.status === 401) signOut(error.message)
      else setBookLoanError(error instanceof Error ? error.message : 'Unable to load this book\'s loan history.')
    } finally {
  ...
  async function handleReserve() {
    ...
    try {
      ...
      await Promise.all([refreshBooks(session), refreshReservations(session), openDetail(selectedBook.id)])
  ...
  async function handleReservationAction(reservation: BookReservation, action: 'confirm' | 'cancel') {
    ...
    try {
      ...
      await Promise.all([refreshReservations(session), refreshLoans(session), refreshBooks(session), refreshNotifications(session)])
    } catch (error) {
      if (error instanceof ReservationsApiError && error.status === 401) signOut(error.message)
      else setReservationError(error instanceof Error ? error.message : 'Unable to update this reservation.')
    } finally {
  ...
  async function handleNotificationRead(notification: Notification) {
    ...
    try {
      ...
      if (error instanceof NotificationsApiError && error.status === 401) signOut(error.message)
      else setNotificationError(error instanceof Error ? error.message : 'Unable to update this notification.')
    }
  }
  ...
  return <main className="app-shell">
    <header className="app-header"><div><p className="eyebrow">Library catalogue</p><h1>Books</h1></div><div className="header-actions"><span className="role-label">{currentRole === 'ADMIN' ? 'Administrator' : 'Member'}</span>{currentRole === 'USER' && <><button className="button-secondary" type="button" onClick={() => { setView('loans'); void refreshLoans() }}>My loans</button><button className="button-secondary" type="button" onClick={() => { setView('reservations'); void refreshReservations(); void refreshNotifications() }}>My reservations</button><button className="button-secondary notification-button" type="button" onClick={() => { setNotificationsOpen(!notificationsOpen); void refreshNotifications() }}>Notifications{notifications.length > 0 ? ` (${notifications.length})` : ''}{notifications.length > 0 && <span className="notification-dot" aria-hidden="true" />}</button></>}<button className="button-secondary" type="button" onClick={() => signOut()}>Sign out</button></div></header>
    {requestError && <div className="error-summary" role="alert" tabIndex={-1}>{requestError}</div>}
     ...
     {view === 'list' && <section aria-labelledby="catalogue-heading"><div className="section-heading"><div><h2 id="catalogue-heading">Catalogue</h2><p className="muted">Browse the library collection and check availability.</p></div>{currentRole === 'ADMIN' && <button type="button" onClick={openCreate}>Add book</button>}</div>
      ...
      {view === 'detail' && <section aria-labelledby="detail-heading"><button className="back-button" type="button" onClick={() => setView('list')}>Back to catalogue</button>{detailLoading && <p className="status" role="status">Loading book...</p>}{detailError && <div className="error-summary" role="alert">{detailError}</div>}{borrowMessage && <p className="success-message" role="status">{borrowMessage}</p>}{selectedBook && <><div className="detail-heading"><div><p className="eyebrow">Book details</p><h2 id="detail-heading">{selectedBook.title}</h2><p className="muted">{selectedBook.author}</p></div>{currentRole === 'ADMIN' && <div className="inline-actions"><button type="button" onClick={openEdit}>Edit</button><button className="button-secondary" type="button" onClick={() => void refreshBookLoans()} disabled={bookLoanLoading}>{bookLoanLoading ? 'Loading loans...' : 'View loan history'}</button><button className="button-danger" type="button" onClick={() => setDeleteConfirm(true)}>Delete</button></div>}{currentRole === 'USER' && <button type="button" onClick={() => void handleBorrow()} disabled={borrowPending || selectedBook.available_copies <= 0}>{borrowPending ? 'Borrowing...' : 'Borrow book'}</button>}</div><dl className="metadata"><div><dt>ISBN</dt><dd>{selectedBook.isbn}</dd></div><div><dt>Publication date</dt><dd>{formatPublicationDate(selectedBook.date)}</dd></div><div><dt>Loan duration</dt><dd>{selectedBook.loan_duration_days} days</dd></div><div><dt>Total copies</dt><dd>{selectedBook.total_copies}</dd></div><div><dt>Availability</dt><dd>{availability(selectedBook).label}</dd></div></dl>{currentRole === 'ADMIN' && bookLoansRequested && <section aria-labelledby="book-loans-heading"><div className="section-heading"><div><p className="eyebrow">Administrator circulation</p><h3 id="book-loans-heading">Loan history</h3><p className="muted">All loans recorded for this book.</p></div>{bookLoans.length > 0 && <button className="button-secondary" type="button" onClick={() => void refreshBookLoans()} disabled={bookLoanLoading}>Refresh</button>}</div>{bookLoanLoading && <p className="status" role="status">Loading loan history...</p>}{bookLoanError && <div className="error-summary" role="alert">{bookLoanError}<button className="button-secondary" type="button" onClick={() => void refreshBookLoans()}>Retry</button></div>}{!bookLoanLoading && !bookLoanError && bookLoans.length === 0 && <div className="empty-state"><h3>No loans found</h3><p>This book has no recorded loans.</p></div>}{!bookLoanLoading && !bookLoanError && bookLoans.length > 0 && <div className="loan-list" role="list">{bookLoans.map((loan) => <article className="loan-card" role="listitem" key={loan.id}><div><h3>Loan #{loan.id}</h3><dl className="loan-meta"><div><dt>Borrowed</dt><dd>{formatLoanDate(loan.loan_timestamp)}</dd></div><div><dt>Due</dt><dd>{formatLoanDate(loan.due_at_timestamp)}</dd></div>{loan.returned_timestamp !== null && <div><dt>Returned</dt><dd>{formatLoanDate(loan.returned_timestamp)}</dd></div>}<div><dt>Member</dt><dd>User #{loan.user_id}</dd></div></dl></div><span className={`availability ${loan.status === LoanStatus.BORROWED ? 'available' : 'unavailable'}`}>{loan.status === LoanStatus.BORROWED ? 'Borrowed' : 'Returned'}</span></article>)}</div>}</section>}</>}
      </section>}
       {view === 'detail' && currentRole === 'USER' && selectedBook && selectedBook.available_copies === 0 && <div className="reservation-callout"><p className="muted">This title is unavailable. Reserve it to join the queue.</p><button type="button" onClick={() => void handleReserve()} disabled={borrowPending || reservations.some((reservation) => reservation.book_id === selectedBook.id && (reservation.status === ReservationStatus.PENDING || reservation.status === ReservationStatus.READY))}>{borrowPending ? 'Reserving...' : reservations.some((reservation) => reservation.book_id === selectedBook.id && reservation.status === ReservationStatus.PENDING) ? 'Reservation pending' : reservations.some((reservation) => reservation.book_id === selectedBook.id && reservation.status === ReservationStatus.READY) ? 'Reservation ready' : 'Reserve book'}</button></div>}
       {view === 'loans' && currentRole === 'USER' && <section aria-labelledby="loans-heading"><div className="section-heading"><div><p className="eyebrow">Member circulation</p><h2 id="loans-heading">My loans</h2><p className="muted">Keep track of books you currently have borrowed.</p></div><div className="inline-actions"><button className="button-secondary" type="button" onClick={() => setView('list')}>Back to catalogue</button><button className="button-secondary" type="button" onClick={() => void refreshLoans()} disabled={loanLoading}>Refresh</button></div></div>{loanLoading && <p className="status" role="status">Loading your loans...</p>}{loanError && <div className="error-summary" role="alert">{loanError}<button type="button" className="button-secondary" onClick={() => void refreshLoans()}>Retry</button></div>}{!loanLoading && !loanError && loans.length === 0 && <div className="empty-state"><h3>No active loans</h3><p>Books you borrow will appear here.</p></div>}{!loanLoading && !loanError && loans.length > 0 && <div className="loan-list" role="list">{loans.map((loan) => { const book = books.find((item) => item.id === loan.book_id); const overdue = loan.status === LoanStatus.BORROWED && loan.due_at_timestamp < currentTimestamp; return <article className="loan-card" role="listitem" key={loan.id}><div><h3>{book?.title ?? `Book #${loan.book_id}`}</h3><p className="muted">{book?.author ?? 'Title details are not available in the current catalogue.'}</p><dl className="loan-meta"><div><dt>Borrowed</dt><dd>{formatLoanDate(loan.loan_timestamp)}</dd></div><div><dt>Due</dt><dd>{formatLoanDate(loan.due_at_timestamp)}</dd></div></dl>{overdue && <span className="availability overdue">Overdue</span>}</div><button type="button" onClick={() => void handleReturn(loan)} disabled={loanPendingId !== null}>{loanPendingId === loan.id ? 'Returning...' : 'Return book'}</button></article> })}</div>}</section>}
       {view === 'reservations' && currentRole === 'USER' && <section aria-labelledby="reservations-heading"><div className="section-heading"><div><p className="eyebrow">Member circulation</p><h2 id="reservations-heading">My reservations</h2><p className="muted">Reserved titles stay in queue until a copy is ready.</p></div><div className="inline-actions"><button className="button-secondary" type="button" onClick={() => setView('list')}>Back to catalogue</button><button className="button-secondary" type="button" onClick={() => { void refreshReservations(); void refreshNotifications() }} disabled={reservationLoading}>Refresh</button></div></div>{reservationError && <div className="error-summary" role="alert">{reservationError}</div>}{reservationLoading && <p className="status" role="status">Loading reservations...</p>}{!reservationLoading && !reservationError && reservations.length === 0 && <div className="empty-state"><h3>No reservations</h3><p>Reservations for unavailable titles will appear here.</p></div>}{!reservationLoading && reservations.length > 0 && <div className="loan-list" role="list">{reservations.map((reservation) => <article className="loan-card" role="listitem" key={reservation.id}><div><h3>{books.find((book) => book.id === reservation.book_id)?.title ?? reservationBookTitles[reservation.book_id] ?? `Book #${reservation.book_id}`}</h3><p className="muted">{formatReservationStatus(reservation.status)}</p>{reservation.status === ReservationStatus.READY && reservation.expires_at_timestamp && <p className="reservation-deadline">Hold until {formatLoanDate(reservation.expires_at_timestamp)}</p>}</div>{(reservation.status === ReservationStatus.READY || reservation.status === ReservationStatus.PENDING) && <div className="inline-actions">{reservation.status === ReservationStatus.READY && <button type="button" onClick={() => void handleReservationAction(reservation, 'confirm')} disabled={reservationPendingId !== null}>{reservationPendingId === reservation.id ? 'Confirming...' : 'Confirm and borrow'}</button>}<button className="button-secondary" type="button" onClick={() => void handleReservationAction(reservation, 'cancel')} disabled={reservationPendingId !== null}>{reservationPendingId === reservation.id ? 'Updating...' : 'Cancel reservation'}</button></div>}</article>)}</div>}</section>}
